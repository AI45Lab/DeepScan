...
class _WebhookSink:
    ...
    def _format_diagnosis_report(self, result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # MI-PEAKS → prefer real evaluator output when present; otherwise keep a stable dummy chart
        # (back-compat + unit tests that don't run MI compute).
        mi_eval = _find_eval(["mi-peaks", "mi_peaks", "mipeaks"])
        mi_metrics_payload: Optional[Dict[str, Any]] = None
        if mi_eval:
            mi_results = mi_eval.get("results") or {}
            mi_metrics = mi_results.get("metrics") or {}
            charts = None
            if isinstance(mi_metrics, dict):
                charts = mi_metrics.get("charts")
                if not charts and isinstance(mi_metrics.get("mi_mean_trajectory"), list):
                    charts = [{"type": "line", "data": mi_metrics.get("mi_mean_trajectory")}]
            if isinstance(charts, list) and charts:
                mi_metrics_payload = {"charts": self._sanitize_numbers(charts)}

        if mi_metrics_payload is None:
            line_chart = [
                0.08,
                0.12,
                0.55,
            ...
            mi_metrics_payload = {"charts": [{"type": "line", "data": line_chart}]}

        results_payload.append({"name": "mi-peaks", "metrics": mi_metrics_payload})
